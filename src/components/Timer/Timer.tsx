...
const Timer = () => {
  ...
  const timerValues = useSelector(
    (state: RootStateOrAny) => state.timerReducer.timerValues
  );
  const inputValues = useSelector(
    (state: RootStateOrAny) => state.timerReducer.timerInputsValues
  );
  ...
  const countdownTimer = (diff: number): Function => {
    ...
    return () => {
      ...
      const hours: string =
        diff > 0 ? formatNumbers(Math.floor(diff / 1000 / 60 / 60) % 24) : '00';
      const minutes: string =
        diff > 0 ? formatNumbers(Math.floor(diff / 1000 / 60) % 60) : '00';
      const seconds: string =
        diff > 0 ? formatNumbers(Math.floor(diff / 1000) % 60) : '00';
  ...
  const handleStart = (event: MouseEvent<HTMLButtonElement>): void => {
    ...
    const timeDifference: number = getDatesDifference(
      +inputValues.hours,
      +inputValues.minutes,
      +inputValues.seconds
    );
  ...
  const timerInputs = Object.entries(inputValues).map(
    ([key, value]: [string, string], index: number): ReactElement => {
      return (
        <input
          key={index}
          value={value}
